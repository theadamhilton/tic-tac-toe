...
export function Board({squares, onPlay}) {
  ...
  function onSquareClick(idx) {
    if (squares[idx] || calculateWinner(squares)) return false;
    ...
    if (idx !== 0 && typeof newSquares[idx - 1] !== 'string') {
      newSquares[idx - 1] = "O";
    } else if (idx !== 0 && typeof newSquares[idx + 1] !== 'string' || idx === 0 && typeof newSquares[idx + 1] !== 'string') {
      newSquares[idx + 1] = "O";
...
export default function Game() {
  ...
  const moves = history.map((squares, move) => {
    ...
    } else if(move === 5 && !calculateWinner(squares)) {
